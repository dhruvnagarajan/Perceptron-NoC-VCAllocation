...
//**************************************************************************************
                ...
                nextvc = getNextVCID(dir,i,flag); // XDEE

                // CVCA cvca;
                // nextvc = cvca.vca(tileID, dir, i);
...
sc_uint<VCS_BITSIZE+1> VCAllocator<num_ip>::getNextVCID (int dir, int dir_from,int flag)
{
    ...
    else if (flag==0)
    {
        ...
        // UI randd = dir+i;
        // srand(randd);
        // cout << "srand:" << randd <<endl;
        if (isAnyVCFree) {
            CVCA cvca;
            UI toReturn = cvca.vca(tileID, dir, vcs);
            vcFree[dir][toReturn] = false;
            return toReturn;
        }
    }
